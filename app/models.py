...
class Volunteer(User):
    ...
    profilePhoto = models.ForeignKey(Documents, null=True, blank=True, on_delete=models.PROTECT, related_name='volunteer_photo')
    isVerified = models.BooleanField(default=False, null=True, blank=True)
...
class FoodItems(models.Model):
    ...
    itemPhotos = models.ManyToManyField(Documents, null=True, blank=True, related_name='food_photos')      
...
class Donations(models.Model):
    ...
    foodItems = models.ManyToManyField(FoodItems, null=True, blank=True, related_name='donation_food_items')
    quantity = models.CharField(max_length=100, default='', null=True, blank=True)   
    address = models.ForeignKey(Address, null=True, blank=True, on_delete=models.PROTECT)
...
class FoodEvents(models.Model):
    foodItems = models.ManyToManyField(FoodItems, null=True, blank=True, related_name='event_food_items')        
    quantity = models.CharField(max_length=100, default='', null=True, blank=True)               
    address = models.ForeignKey(Address, null=True, blank=True, on_delete=models.PROTECT)
    organizerPhoneNumber = models.CharField(max_length=20, default='', null=True, blank=True)
    pickupDate = models.DateTimeField(null=True, blank=True) 
    eventPhotos = models.ManyToManyField(Documents, null=True, blank=True, related_name='event_photos')
...
class FoodRecipes(models.Model):
    ...
    foodType = models.CharField(max_length=50, choices=FOOD_TYPE, null=True, blank=True) 
    category = models.CharField(max_length=50, choices=CATEGORY, null=True, blank=True) 
    foodImage = models.ManyToManyField(Documents, null=True, blank=True, related_name='recipe_photos') 
